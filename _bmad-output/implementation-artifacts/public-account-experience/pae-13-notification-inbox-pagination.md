...
- [x] **API** — `NotificationController.listInbox`: thêm `@RequestParam page`, `limit`; map `PaginationResponse`; thêm `meta.unreadCount`.
- [x] **Service** — `NotificationInboxService.listInbox(userId, page, limit)`: merge + sort như hiện tại, tính `total` / `unreadCount` trước slice; giữ `MAX_ITEMS` aggregate (50) là trần an toàn — phân trang áp dụng trên danh sách đã merge (tối đa 50 mục, tức tối đa 5 trang với `limit=10`).
- [x] **Shared** — (tuỳ chọn) type `PaginationMeta` / mở rộng response type nếu pattern shared đã có; đồng bộ `ApiPaths` nếu thêm query documented.
- [x] **Web hook** — Tách hoặc mở rộng `useNotificationInbox`:
  - Mode **summary** (bell): `page=0`, `limit=5`, dùng `meta.unreadCount`.
  - Mode **paged** (settings): state `page` + `limit=10`, query key gồm page.
- [x] **UI** — `NotificationInboxList` variant `full`: nhận `pagination`, `onPageChange`, render footer phân trang (reuse UX audit log: nút disabled, `aria-label` tiếng Việt).
- [x] **UI** — `/settings/notifications/page.tsx`: wire phân trang; giữ email preferences section không đổi.
- [x] **Tests** — Cập nhật `NotificationControllerTest`, `NotificationInboxServiceTest` (pagination bounds, unreadCount, empty page).

### Review Findings

- [x] [Review][Patch] Optimistic read/read-all only patches one inbox query cache [apps/web/src/hooks/useNotificationInbox.ts:113]
...
- `cd apps/api && ./gradlew test --tests com.healthlens.api.controller.NotificationControllerTest --tests com.healthlens.api.service.NotificationInboxServiceTest` — bị chặn do máy không có Java Runtime.
- `cd packages/shared && pnpm build` — pass.
- `cd apps/web && pnpm test` — pass.
- `cd apps/web && pnpm exec tsc -p tsconfig.json --noEmit` — fail do lỗi tồn tại sẵn trong `.next` generated validators và `src/lib/seo/site.test.ts` gán `NODE_ENV` readonly; không phát sinh từ story này.

### Completion Notes List

- Đã thêm DTO `NotificationInboxPageResponse` và API `GET /notifications/inbox?page=&limit=` trả `data`, `pagination`, `meta.timestamp`, `meta.requestId`, `meta.unreadCount`.
- `NotificationInboxService` giữ aggregate `MAX_ITEMS=50`, tính `total`/`unreadCount` trước khi slice, chuẩn hóa `page >= 0`, `limit <= 50`, và trả mảng rỗng khi page vượt `totalPages`.
- `useNotificationInbox` hỗ trợ `summary` cho chuông (`page=0`, `limit=5`) và `paged` cho settings (`limit=10`, query key gồm page); optimistic mark-read/read-all vẫn invalidate toàn bộ inbox queries.
- `NotificationInboxList` full variant có footer phân trang tiếng Việt và settings page đã nối state page; email preferences không đổi.
- Chưa thể chuyển story sang `review` vì backend Gradle test bị chặn bởi thiếu Java Runtime trên máy.

### File List

- apps/api/src/main/java/com/healthlens/api/controller/NotificationController.java
- apps/api/src/main/java/com/healthlens/api/dto/response/NotificationInboxPageResponse.java
...
- _bmad-output/implementation-artifacts/public-account-experience/pae-13-notification-inbox-pagination.md

### Change Log

- 2026-05-25: Story created — pagination cho trang Cài đặt → Thông báo (PAE-13).
- 2026-05-28: Implemented inbox pagination API, hook modes, settings pagination UI, and backend test coverage; backend validation blocked by missing Java Runtime.
