...
export const ConsentModal: React.FC = () => {
  ...
  const shouldHideOnAuthRoutes = hiddenPaths.some((path) =>
    pathname?.startsWith(path),
  );
  const shouldHideOnPublicMarketingRoutes = publicMarketingPaths.some(
    (path) => {
      if (path === "/") {
        return pathname === "/";
      }
  ...
  if (
    !isAuthenticated ||
    consentGiven ||
    shouldHideOnAuthRoutes ||
    shouldHideOnPublicMarketingRoutes
  ) {
    return null;
  }
  ...
  const handleAccept = async () => {
    setSubmitting(true);
    try {
      await apiClient.post(ApiPaths.CONSENT.ME, {
      ...
      // Check known Axios error details when available, while keeping user-facing messages friendly.
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      notify.info(
        "Bạn đã từ chối đồng thuận. Vui lòng đăng nhập lại để tiếp tục.",
      );
    } catch {
      ...
      <div className="w-full max-w-[700px] max-h-[calc(100vh-1rem)] sm:max-h-[calc(100vh-2rem)] overflow-hidden rounded-xl bg-white shadow-[0_24px_80px_rgba(0,0,0,0.25)] flex flex-col">
        <div className="p-6 sm:p-8 text-center text-white bg-linear-to-r from-[#00685f] to-[#008378]">
          <h2 className="text-2xl font-bold tracking-tight">
            Điều khoản & đồng thuận dữ liệu y tế
          </h2>
        ...
        <div className="bg-white p-6 sm:p-8 overflow-y-auto hl-custom-scrollbar">
          <div className="space-y-8 pr-2 text-[15px] leading-relaxed text-slate-700">
            <section>
              <h3 className="text-lg font-semibold text-[#00685f] mb-3">
                1. Mục đích thu thập
              </h3>
              <p className="text-slate-600">
                HealthLens thu thập dữ liệu sức khỏe bạn cung cấp (kết quả xét
                nghiệm, chỉ số sức khỏe) để hỗ trợ giải thích, theo dõi và cá
                nhân hóa trải nghiệm sử dụng.
              </p>
            </section>

            <section>
              <h3 className="text-lg font-semibold text-[#00685f] mb-3">
                2. Thu thập & xử lý dữ liệu (NĐ 13/2023/NĐ-CP)
              </h3>
              <div className="space-y-4 text-slate-600">
                <div className="rounded-lg bg-[#e9f6f3] p-4">
                  <p className="font-semibold text-[#3f6560] mb-1">
                    Loại dữ liệu
                  </p>
                  <p>
                    Dữ liệu y tế do bạn tải lên/cung cấp và một số thông tin kỹ
                    thuật phục vụ bảo mật.
                  </p>
                </div>
                <div>
                  <p className="font-semibold text-[#3f6560] mb-1">
                  ...
                  <p>
                    Hiển thị kết quả, giải thích thông tin y khoa, và cải thiện
                    chất lượng dịch vụ.
                  </p>
                </div>
                <div>
                  ...
                  <p>
                    Bạn có quyền truy cập, chỉnh sửa, yêu cầu xóa hoặc phản đối
                    việc xử lý dữ liệu theo quy định pháp luật.
                  </p>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="flex items-start gap-3 rounded-lg bg-[#deebe8] p-4">
                <ShieldCheck
                  className="mt-0.5 h-5 w-5 text-[#00685f]"
                  aria-hidden="true"
                />
                <p className="text-sm text-slate-600">
                  Dữ liệu được mã hóa (AES-256) và chỉ sử dụng cho mục đích cung
                  cấp dịch vụ. HealthLens cam kết không chia sẻ với bên thứ ba
            ...
            <section>
              ...
              <div className="flex items-start gap-3 rounded-r-lg border-l-4 border-[#924628] bg-[#ffdbce] p-4 text-[#3d4947]">
                <TriangleAlert
                  className="mt-0.5 h-5 w-5 shrink-0 text-[#924628]"
                  aria-hidden="true"
                />
                <p className="font-medium">
                  Khuyến nghị y tế từ HealthLens chỉ mang tính tham khảo, không
                  thay thế tư vấn từ bác sĩ chuyên môn.
                </p>
              </div>
            </section>
          </div>
          ...
          <div className="mt-6 border-t border-slate-200 pt-6 space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer min-h-[44px] select-none">
              ...
              />
              <span className="text-slate-700">
                Tôi đồng ý cho HealthLens thu thập và xử lý dữ liệu sức khỏe
