...
function parseApiError(error: unknown) {
  if (
    ...
    return response.data?.detail || "Không thể xử lý yêu cầu.";
  }
...
export default function ReferenceDataImportPage() {
  ...
  const previewMutation = useMutation({
    mutationFn: async (targetFile: File) => {
      ...
      const response = await adminApiClient.post(API_ROUTES.ADMIN_REFERENCE_DATA.IMPORT_PREVIEW, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      return response.data.data as PreviewResponse;
    },
    onSuccess: (data) => {
      setPreview(data);
      setNotice(null);
    ...
    onSuccess: async (data) => {
      ...
      setNotice({
        ...
        message: multiAdminMode
          ? `${data.message} Đã gửi ${data.draftChangeSetCount} change set sang hàng chờ phê duyệt.`
          : `${data.message} Đã tạo ${data.draftChangeSetCount} bản nháp.`,
      });
      setPreview(null);
      setFile(null);
      if (inputRef.current) inputRef.current.value = "";

      router.push("/admin/reference-data/approvals");
    },
    onError: (error) => {
      setNotice({ type: "error", message: parseApiError(error) });
    },
  ...
  const handleFileChange = async (nextFile: File | null) => {
    ...
    setPreview(null);
    setNotice(null);
    await previewMutation.mutateAsync(nextFile);
  };
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-teal-700">
              ...
              Import dữ liệu tham chiếu
            </div>
            <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-900">
              Import dữ liệu tham chiếu
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
            Upload file CSV/JSON để hệ thống parse, validate và hiển thị dữ liệu trước khi áp dụng. Sau khi xác nhận, dữ liệu sẽ được đưa vào hàng chờ phê duyệt.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">Valid rows</h2>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">Error rows</h2>
