...
export default function ProfileHistoryPage() {
  ...
  const historyQuery = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      const response = await apiClient.get(ApiPaths.PROFILES.HEALTH_RECORDS(profileId), {
  ...
  const currentProfileName = useMemo(
    () => profiles.find((profile) => profile.id === profileId)?.displayName ?? "Hồ sơ",
    [profiles, profileId],
  );
        ...
        <section className="mt-3 space-y-4">
          <article className="rounded-2xl border border-[#b7d8d1] bg-white p-6 shadow-sm">
            ...
            <div className="mb-4 grid grid-cols-1 divide-y divide-[#e8eeec] overflow-hidden rounded-2xl border border-[#dde6e3] bg-[#f8fbfa] lg:grid-cols-4 lg:divide-x lg:divide-y-0">
              <label className="flex min-h-[52px] items-center gap-2.5 px-5 text-sm font-medium text-[#7b8a87]">
            ...
            <div className="overflow-hidden rounded-xl border border-[#e0ece9]">
              ...
              {filteredItems.length > 0 ? (
                filteredItems.map((item) => (
                  ...
                  >
                    <span className="text-[#3d4947]">{item.examDate || "Chưa có ngày khám"}</span>
                    <div className="space-y-1">
                      <p className="font-semibold text-[#005049]">{item.testType || "Phiếu khám bệnh"}</p>
            ...
            <div className="mt-4 flex flex-col gap-3 text-sm text-[#6d7a77] sm:flex-row sm:items-center sm:justify-between">
              <span>
                Đang hiển thị {filteredItems.length} trong số {allItems.length} kết quả đã tải
              </span>
              {hasNextPage ? (
                <button
                  type="button"
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[2fr_1fr]">
            <article className="relative overflow-hidden rounded-2xl border border-[#d5e7e3] bg-[#eaf9f5] p-7 shadow-sm">
              ...
              <p className="mt-3 max-w-2xl text-[28px] leading-relaxed text-[#41716b]">
                Dựa trên kết quả gần nhất, chúng tôi khuyên bạn nên duy trì chế độ ăn ít tinh bột và tái khám sau 3 tháng.
              </p>
              <button
                type="button"
            ...
            <article className="rounded-2xl border border-[#e5edeb] bg-white p-6 text-center shadow-sm">
              ...
              <h3 className="text-2xl font-bold text-[#253c39]">Kết quả mới nhất?</h3>
              <p className="mt-4 text-lg leading-relaxed text-[#5e7471]">
                Bạn có thêm kết quả xét nghiệm từ bệnh viện khác? Hãy cập nhật để HealthLens phân tích toàn diện hơn.
              </p>
              <button
                type="button"
          ...
          <div ref={loadMoreRef} className="flex h-12 items-center justify-center">
            {isFetchingNextPage ? (
              ...
              <span className="text-sm text-[#6d7a77]">Kéo xuống để tải thêm</span>
            ) : null}
          </div>
        </section>
      )}
